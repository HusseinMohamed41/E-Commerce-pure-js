...
// If there are products saved in localStorage, draw them on page load
if (proudectInCart) {
...
function drawProudectCart(products) {
  let productCards = products.map((item) => {
    // Get saved quantity from localStorage (default = 1)
    let quantity = +(localStorage.getItem(`quantity-${item.id}`)) || 1;

    // Return HTML for each product card
    return `
...
// Recalculate total price when page loads
if (addItemStorage) {
  total = 0;
...
function removeFromCart(id) {
  ...
  if (itemIndex !== -1) {
    ...
    // recalculate total using correct quantity for each item
    addItemStorage.forEach((item) => {
      const itemQuantity = +(localStorage.getItem(`quantity-${item.id}`)) || 1;
      total += +item.salePrice * itemQuantity;
    });

    // Update total price in DOM & localStorage
    totalPrice.innerHTML = total;
...
function mins(id, salePrice) {
  ...
  if (quantity > 1) {
    ...
    // If quantity goes below 1 → remove product completely
    removeFromCart(id);
  }
}
...
function drawFavData() {
  ...
  for (let i = 0; i < favorites.length; i++) {
    ...
    if (item) {
      ...
      // Create a new carousel slide every 3 items
  ...
  // Insert slides & indicators into DOM
